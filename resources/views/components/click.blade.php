 <div
     class= "inline-block relative text-left bg-gray-100 rounded-lg overflow-hidden align-bottom transition-all transform
        shadow-2xl sm:my-8 sm:align-middle sm:max-w-xl sm:w-full grid">
     <button type="button" class="text-black text-xl size-10 rounded bg-gray-50 flex items-center justify-center hover:text-gray-600 absolute top-5 right-5" @click="selectedUser = null; isVerified = false">
         &times;
     </button>
     <div class="flex flex-col items-center pt-6 pr-6 pb-6 pl-6">
         <div
             class="flex bg-gray-50 text-base tracking-wide uppercase text-black items-center justify-center border-2 border-gray-200 size-16  mr-auto -mb-8 ml-auto rounded-full">
         ...
         <form action="{{ route('visitor.resend.visit.request') }}" method="POST" class="w-full gap-4 grid mt-6">
             ...
             <textarea placeholder='You have any reason to visit us?' name="reason" rows="4"
                 class="w-full resize-none text-black bg-transparent rounded-md px-4 border border-gray-300 text-sm pt-2.5 outline-0 focus:border-black"></textarea>
