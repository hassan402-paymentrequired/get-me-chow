...
@section('contents')
    <div class="grid md:grid-cols-3 gap-5 p-10">
        @foreach ($user->visitorCHeck as $check)
            <div class="overflow-hidden rounded-xl border border-gray-200">
                <div class="flex items-center gap-x-4 border-b border-gray-900/5 bg-gray-50 p-6">
                    <div
                        ...
                        {{ substr($check->visitor->name, 0, 2) }}
                    </div>
                    <div class="text-sm/6 font-medium text-gray-900">{{ $check->visitor->name }}</div>
                ...
                <dl class="-my-3 divide-y divide-gray-100 px-6 py-4 text-sm/6">
                    <div class="flex justify-between gap-x-4 py-3">
                        ...
                        <dd class="text-gray-700"><time
                                datetime="2022-12-13">{{ $check->visitor->latestCheckin->user->first_name . ' ' . $check->visitor->latestCheckin->user->last_name }}</time>
                        </dd>
                    </div>
                    <div class="flex justify-between gap-x-4 py-3">
                        <dt class="text-gray-500">Check in time</dt>
                        <dd class="flex items-start gap-x-2">
                            <div class="font-medium text-gray-900">{{ $check->visitor->created_at->format('h:i A') }}</div>
                            <div @if ($check->visitor->latestCheckin->check_out_time) class="rounded-md bg-red-50 px-2 py-1 text-xs font-medium text-red-700 ring-1 ring-red-600/10 ring-inset">
                                    Check out
                                    </div>
    ...
    @if ($user->visitorCHeck->count() === 0)
        <div class="flex items-center justify-center w-full ">
            <p class="text-gray-500">No visitor found</p>
        </div>
    @endif
@endsection
